...
if ($_POST) {
    ...
    if ($exists) {
        echo "<div style='text-align:center; padding:50px;'><h2 style='color:red;'>Ошибка!</h2><p>Волонтёр с телефоном $phone уже зарегистрирован.</p><a href='volunteers.php' class='btn'>Вернуться</a></div>";
        exit;
    }

    addVolunteer($pdo, $name, $phone, $skill, $photo_url);
    echo "<div style='text-align:center; padding:50px;'><div style='background:#e8f5e9; max-width:400px; margin:0 auto; padding:30px; border-radius:20px;'><h2 style='color:green;'>Спасибо, $name!</h2><p>Вы теперь в команде волонтёров!</p><a href='volunteers.php' class='btn'>Вернуться</a></div></div>";
    exit;
}
?>
<!doctype html>
